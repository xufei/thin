...
Java飘过：“我是纯面向对象语言哦，但我有class loader哦”，“死开！”
...
我们看AngularJS框架，写了一个类似的代码：

	thin.module("A", [], function() {
		//module A
	});

	thin.module("B", ["A"], function(A) {
		//module B
		var a = new A();
	});
...
我们可以看到，这里面递归获取了依赖项，然后当作参数，用这个模块的工厂方法来实例化了一下。以此类推，假如一个模块有多个依赖项，也可以用类似的方式写，毫无压力：

	thin.module("D", ["A", "B", "C"], function(A, B, C) {
		//module D
		var a = new A();
		var b = new B();
		var c = new C();
...
注意了，D模块的工厂，实参的名称未必就要是跟依赖项一致，比如，以后我们代码较多，可以给依赖项和模块名称加命名空间，可能变成这样：
	...
	thin.module("foo.D", ["foo.A", "foo.B", "foo.C"], function(A, B, C) {
		//module D
		var a = new A();
		var b = new B();
		var c = new C();
...
测试代码如下：
	...
	thin.module("constant.PI", [], function() {
		return 3.14159;
	});

	thin.module("shape.Circle", ["constant.PI"], function(pi) {
	...
	thin.module("shape.Rectangle", [], function() {
	...
	thin.module("ShapeTypes", ["shape.Circle", "shape.Rectangle"], function(Circle, Rectangle) {
		return {
			CIRCLE: Circle,
			RECTANGLE: Rectangle
		};
	...
	thin.module("ShapeFactory", ["ShapeTypes"], function(ShapeTypes) {
		return {
	...
	var ShapeFactory = thin.get("ShapeFactory");
